...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Function to handle data submission (calls API)
    const saveData = async () => {
        ...
        const id = generateNumericID().toString();

        try {
            await TaskAPI.createTask(id, taskDescription);
            textInput.value = "";
            dateInput.value = "";
            await displayTasks(currentSection);
        } catch (err) {
            console.error('Create task failed', err);
            swal({ title: 'Error', text: 'Unable to create task (see console)', icon: 'error' });
        }
    };
    ...
    // Function to retrieve tasks from API (or provided list) and display them
    const displayTasks = async (section, tasksToDisplay) => {
        ...
        if (!tasksToDisplay) {
            try {
                ...
                console.error('Failed to fetch tasks, falling back to localStorage', err);
        ...
        taskContainer.addEventListener("click", async (event) => {
            ...
            // Handle delete icon click (calls API)
                ...
                if (task) {
                    ...
                    document.getElementById('hiddenDatePicker').addEventListener('change', function () {
                        ...
                        swal({
                            ...
                            if (willChangeDate) {
                                // Update date locally and refresh UI. API update not implemented.
                                task.date = selectedDate;
                                // If task has id persisted locally, update localStorage for UX
                                try {
    ...
    logoutLink.addEventListener("click", (e) => {
        ...
        swal({
            ...
            if (willLogout) {
                // Call the logout function from index.html
                if (typeof handleCognitoLogout === 'function') {
                    handleCognitoLogout();
                } else {
                    // Fallback logout
                    localStorage.clear();
                    window.location.href = 'http://vibetrack.s3-website.ap-south-2.amazonaws.com/login.html';
                }
            }
        });
    });
    ...
    // Disable old popup - user login via Cognito instead
    // if (
